module Arkswarm
  module ConfigLoader
    ...
    # Loads up configurations that are found at the provided location
    ...
    def self.update_cfg_value(ini_file_hash, key, value, add = true)
      update_status = false
      ini_file_hash.each do |_section, values|
    ...
    def self.generate_config_file(filehash, file_location = nil)
      ...
      if file_location # write out the content if we are provided a file location
        File.open(file_location.to_s, 'w+') do |file|
